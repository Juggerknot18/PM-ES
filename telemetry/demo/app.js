  ...
  constructor(el, opts) {
    ...
    el.innerHTML = `
      <svg viewBox="0 0 120 108">
        ...
        <path class="arc" pathLength="100" d="${this._arc()}" stroke-dasharray="0 100"/>
...
const gauges = {
  vbus: new Gauge($("g-vbus"), { min: 0, max: 100, unit: "%", label: "DC link",
    decimals: 1, sub: "relative level", zones: [[0, C.ok], [76, C.warn], [90, C.fault]] }),
  ...
  brake: new Gauge($("g-brake"), { min: 0, max: 4, unit: "", label: "Protection", decimals: 0, sub: "" }),
  can: new Gauge($("g-can"), { min: 0, max: 1, unit: "", label: "Controller link" }),
  spi: new Gauge($("g-spi"), { min: 0, max: 12, unit: "Hz", label: "Demo update", decimals: 1, sub: "browser generated" }),
};

const chVbus = new Chart($("ch-vbus"),
  [{ key: "dc_link_pct", label: "relative level", color: C.info, width: 2 }],
  { minSpan: 8 });
...
function legend(el, chart) {
  el.innerHTML = chart.series.map(s => `<span><i style="background:${s.color}"></i>${s.label}</span>`).join("");
}
legend($("lg-rpm"), chRpm);
legend($("lg-err"), chErr);
...
function render(snap) {
  ...
  setBadge($("b-spi"), st.link_online ? "DATA LINK ONLINE" : "DATA LINK OFFLINE", st.link_online ? "ok" : "fault");
  setBadge($("b-mode"), "PUBLIC DEMO", "mode");
  $("btn-sim").textContent = `Scenario: ${st.scenario}`;
  $("btn-sim").classList.add("on");

  if (f) {
    setBadge($("b-fesc"), f.controller_online ? "CONTROLLER ONLINE" : "CONTROLLER OFFLINE", f.controller_online ? "ok" : "fault");
    setBadge($("b-enc"), f.encoder_healthy ? "ENCODER OK" : "ENCODER ALERT", f.encoder_healthy ? "ok" : "fault");
    setBadge($("b-index"), f.reference_valid ? "REFERENCE VALID" : "REFERENCE PENDING", f.reference_valid ? "ok" : "warn");
    ...
    setBadge($("b-motor"), "MACHINE " + f.motor_state_txt,
      f.motor_state_txt === "FAULT" ? "fault" : f.motor_state_txt === "RUNNING" ? "ok" : f.motor_state_txt === "LIMITED" ? "warn" : "info");

    gauges.vbus.set(f.dc_link_pct);
    gauges.rpm.set(Math.abs(f.measured_rpm), null, f.measured_rpm < 0 ? "reverse direction" : "illustrative encoder");
    ...
    gauges.can.setText(f.controller_online ? "ON" : "OFF", f.controller_online ? C.ok : C.fault, `alerts ${f.controller_alerts}`);

    $("sy-vbus").textContent = f.dc_link_pct.toFixed(1) + " %";
    ...
    setNode("nd-bus", f.dc_link_pct >= 90 ? "fault" : f.dc_link_pct >= 76 ? "warn" : "ok");
    setNode("nd-fsesc", !f.controller_online || f.motor_state_txt === "FAULT" ? "fault" : "ok");
    setNode("nd-motor", f.motor_state_txt === "FAULT" ? "fault" : f.motor_state_txt === "LIMITED" ? "warn" : "ok");
    setNode("nd-enc", "small " + (!f.encoder_healthy ? "fault" : f.reference_valid ? "ok" : "warn"));
    setNode("nd-brake", "small " + (f.protection_level >= 3 ? "fault" : f.protection_level >= 1 ? "warn" : "ok"));
  ...
  $("events").innerHTML = [...snap.events].reverse().map(e => {
    ...
    return `<li class="${e.level}"><span class="e-ts">${ts}</span><span class="e-src">${e.source}</span><span class="e-msg">${e.msg}</span></li>`;
  }).join("");
  ...
  $("s-age").textContent = st.last_valid_ts ? ((Date.now() / 1000 - st.last_valid_ts).toFixed(1) + " s") : "—";
  const check = $("s-crcok");
...
function renderDetail(f) {
  $("detail").innerHTML = DETAIL_MAP.map(([k, fn]) =>
    fn === null ? `<span class="sep">${k}</span>` : `<span class="k">${k}</span><span class="v">${fn(f)}</span>`
  ).join("");
}
...
function connect() {
  ...
  ws.onmessage = (ev) => {
    ...
    if (msg.history) {
      chVbus.seed(msg.history);
      chRpm.seed(msg.history);
      chErr.seed(msg.history);
      return;
    }
    render(msg);
  };
